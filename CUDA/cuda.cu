...
//region Directions
bool inline has_up(int bin_id, int bins_per_row) {
    return bin_id - bins_per_row > -1;
}
bool inline has_down(int bin_id, int bins_per_row, int _bin_count) {
    return bin_id + bins_per_row < _bin_count;
}
bool inline has_left(int bin_id, int bins_per_row) {
    return bin_id % bins_per_row != 0;
}
bool inline has_right(int bin_id, int bins_per_row) {
    return bin_id % bins_per_row != bins_per_row - 1;
}
//endregion

__device__ int* get_neighbours(int* _bin_map, int bin_id) {
    const int NEIGHBOURS_SIZE = 8;
    return &_bin_map[bin_id * NEIGHBOURS_SIZE];
}
...
//__global__ void init_bin_neighbours1(int _bin_count, int** _bin_neighbours_map, int bins_per_row) {
...
void init_bin_neighbours(int _bin_count, int* _bin_neighbours_map, int bins_per_row) {
    const int NEIGHBOURS_SIZE = 8;

    for(int bin_id = 0; bin_id < _bin_count; ++bin_id) {
        int count = 0;

        if (has_up(bin_id, bins_per_row)) {
            _bin_neighbours_map[bin_id * NEIGHBOURS_SIZE + count] = bin_id - bins_per_row;
            count++;
        }
        // up right
        if (has_up(bin_id, bins_per_row) && has_right(bin_id, bins_per_row)) {
            _bin_neighbours_map[bin_id * NEIGHBOURS_SIZE + count] = bin_id - bins_per_row + 1;
            count++;
        }
        // right
        if (has_right(bin_id, bins_per_row)) {
            _bin_neighbours_map[bin_id * NEIGHBOURS_SIZE + count] = bin_id + 1;
            count++;
        }
        // down right
        if (has_down(bin_id, bins_per_row, _bin_count) && has_right(bin_id, bins_per_row)) {
        ...
        for (; count < NEIGHBOURS_SIZE; ++count) {
            _bin_neighbours_map[bin_id * NEIGHBOURS_SIZE + count] = -1;
        }
    }
}
...
__global__ void compute_forces_gpu(particle_t* particles, int num_parts, const int* _bin_map, int* _bin_neighbours_map) {
    ...
    const int* neighbours = get_neighbours(_bin_neighbours_map, bin_id);// _bin_neighbours_map[bin_id];
    const int NEIGHBOURS_SIZE = 8;

    for (int neighbour_part_id = 0; neighbour_part_id < num_parts; neighbour_part_id++) {

//        printf("NEIGHBOUR %d\n", neighbour_part_id); 0.044858136175981354 0.06821978371702932
        int neighbour_bin_id = _bin_map[neighbour_part_id];
//        if(neighbour_bin_id == bin_id || find_bin_id(neighbours, NEIGHBOURS_SIZE, neighbour_bin_id)) {
...
__global__ void rebin(particle_t* particles, double _bin_size, int _bin_row_count, int num_parts, int* _bin_map) {
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid >= num_parts)
        return;
...
//    int bin_blks = (bin_count + NUM_THREADS - 1) / NUM_THREADS;
    ...
    const int NEIGHBOURS_SIZE = 8;
    int* H_bin_neighbours_map = new int[bin_count * NEIGHBOURS_SIZE];
    init_bin_neighbours(bin_count, H_bin_neighbours_map, bin_row_count);

    cudaMalloc((void**)&bin_neighbours_map, bin_count * NEIGHBOURS_SIZE * sizeof(int));

    cudaMemcpy(bin_neighbours_map, H_bin_neighbours_map, bin_count * NEIGHBOURS_SIZE * sizeof(int), cudaMemcpyHostToDevice);
    delete[] H_bin_neighbours_map;
