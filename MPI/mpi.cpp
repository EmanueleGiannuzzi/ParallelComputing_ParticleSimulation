...
void start_send_to_neighbours(int my_rank, int num_procs, const int* _focus_ids, vector<MPI_Request*>& requests, vector<particle_t*>& send_buffers) {
    for(int i = 0; i < focus_count; ++i)  {
        const bin_t* focus = &bin_data[_focus_ids[i]];
...
bool is_my_focus(const int* _focus_ids, int bin_id) {
    for(int i = 0; i < focus_count; ++i) {
        if(bin_data[focus_ids[i]].id == bin_id){
            return true;
        }
    }
    return false;
...
int get_receive_count(const int* _focus_ids) {
    int count = 0;
    for(int i = 0; i < focus_count; ++i) {
        const bin_t *focus = &bin_data[_focus_ids[i]];
        for(int j = 0; j < focus->neighbours_size; ++j) {
            int neighbor_id = focus->neighbours[j]->id;
            if(!is_my_focus(_focus_ids, neighbor_id)) {
                count++;
            }
        }
    }
...
void start_receive_from_neighbours(int my_rank, const int* _focus_ids, particle_t* parts, int particle_count) {
    int receive_count = 0;
    int max_receive_count = get_receive_count(_focus_ids);
...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    start_send_to_neighbours(rank, num_procs, focus_ids, send_requests, send_buffers);
//    clear_requests_ready_only(send_requests, send_buffers);

    printf("-----------RANK %d Done sending\n", rank);

    start_receive_from_neighbours(rank, focus_ids, parts, num_parts);//Blocking
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    if(rank != 0) {
        ...
        for(int i = 0; i < focus_count; ++i)  {
            const bin_t* focus = &bin_data[focus_ids[i]];
